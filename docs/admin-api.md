...
```jsonc
{
  ...
  "stat_lines": [
    { "player_id": "…", "team_id": "…", "is_goalie": true, "minutes_played": 15 },
...
**`minutes_played`** — необязательные минуты вратаря в этом матче. Нужны, когда у команды
было двое вратарей: броски и голы из табло между ними не делятся, а время известно.
Пусто — минуты считаются по регламенту турнира и только для единственного вратаря.
