...
> ⚠️ Показатели могут включать **поправку на историю вне системы** — см. подводный камень 6.
...
### 6. Статистика команды = матчи в системе + поправка на историю

В `GET /teams` и `GET /teams/{id}` показатель может включать **поправку**: матчи старых
турниров, которых в системе ещё нет, администратор задаёт вручную.

Модель складывающая, а не замещающая: хранится разница, а не итог. Поэтому новые
заведённые матчи **продолжают прибавляться** к итогу автоматически, и цифра не устаревает.
Публичный API отдаёт только итог и не раскрывает, где есть поправка. Следствия:

- общая статистика команды может быть больше суммы по её матчам в системе — это не ошибка,
  а как раз учтённая история;
- `games` не обязан равняться `wins + draws + losses`, если поправка задана лишь части полей;
- `points` всегда согласованы с `wins` и `draws` того же ответа: очки не задаются вручную,
  а выводятся из итоговых побед и ничьих.

Если нужна заведомо только «системная» арифметика — считайте её из
`/tournaments/{id}/standings` и `/tournaments/{id}/games`: там поправок нет.
